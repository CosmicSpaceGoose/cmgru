...
class Model_Account extends Model {
	...
	private function update_pass() {
		...
		if ( hash( 'whirlpool', $request[0]['email'].$_POST['oldpass'] ) != $request[0]['password'] )
		...
		db_query_update(
				"`users`",
				array( "password" ),
				array( "password" => hash('whirlpool', $request[0]['email'].$_POST['newpass'] )),
				"`username`='".$_SESSION['loggued_on_user']."'"
		);
		return ( null );
	}
	...
	public function resent_mail() {
		...
		$link = "http://localhost:8102/auth/verify?mail=".$request['email']."&ps=".$request['password'];
