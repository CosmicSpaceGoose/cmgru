...
class Model_Auth extends Model {
	...
	private function validate_login() {
		...
		if (!filter_var( $cook, FILTER_VALIDATE_EMAIL ))
		{
			$data['logErr'] = "Invalid e-mail adress.";
			$data['frm'] = 'login';
			return ( $data );
		}
		...
		if ( isset( $request[0] ))
		{
			if ($request[0]['password'] == hash('whirlpool', $_POST['password']))
			{
				$_SESSION["loggued_on_user"] = $request[0]['username'];
				$_SESSION["uid"] = $request[0]['userId'];
			}
			else
	...
	private function reset_req() {
		...
		else
		{
			...
			if ( isset( $request[0] )) {
				$link = "http://".$_SERVER["HTTP_HOST"]."/auth/vereset?mail=".$mail."&ps=".hash('whirlpool', $request[0]['userId'].$request[0]['password'] );
				$message = "
		...
		if ( count( $request ) > 0 ) {
			if ( hash( 'whirlpool', $request[0]['userId'].$request[0]['password'] == $_GET['ps'] ) ) {
				...
				db_query_update(
					"`users`",
					array( "password" ),
					array( "password" => hash( 'whirlpool', $new )),
					"`email`='".$_GET['mail']."'"
				);
				return ( true );
			}
